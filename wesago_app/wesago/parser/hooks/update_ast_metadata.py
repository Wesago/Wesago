...
class UpdateAstMetadataHookAction(Protocol):
    """
    A standard Wesago function used to update the metadata with data from the Abstract
    ...
    A `dict` with metadata with some keys already pre-set by Wesago and
    previous plugins.
...
class UpdateAstMetadataHook(
    ...
    """
    This hook wraps the standard function that Wesago uses to update the metadata
    ...
    ```python
    from wesago.parser.context import ParserContext
    from wesago.threads.models import Thread
