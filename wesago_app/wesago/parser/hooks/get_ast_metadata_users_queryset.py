...
class GetAstMetadataUsersQuerysetHookAction(Protocol):
    """
    A standard Wesago function used to create a `User` queryset or the next filter
    ...
    Names are normalized using the `slugify` function from `wesago.core.utils`.
...
class GetAstMetadataUsersQuerysetHook(
    ...
    """
    This hook wraps the standard function that Wesago uses to retrieve the metadata
