...
@pytest.mark.parametrize(
    "link",
    (
        "https://wesago-project.org/",
        "http://wesago-project.org/",
        "http://wesago-project.org",
        "wesago-project.org",
        "wesago-project.org/some-path/",
    ),
)
def test_forum_address_is_inbound_link_returns_true_for_inbound_link(link):
    forum_address = ForumAddress(Mock(forum_address="https://wesago-project.org"))
...
@override_settings(WESAGO_FORUM_ADDRESS_HISTORY=["http://example.org"])
