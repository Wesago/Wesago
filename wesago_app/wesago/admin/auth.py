...
TOKEN_KEY = "wesago_admin_session_token"
UPDATED_KEY = "wesago_admin_session_updated"
...
def is_admin_authorized(request):
    ...
    if not request.user.is_wesago_admin:
        return False
    ...
    if updated < time() - (settings.WESAGO_ADMIN_SESSION_EXPIRATION * 60):
        if updated:
...
def django_login_handler(sender, **kwargs):
    ...
    if admin_namespace and user.is_wesago_admin:
