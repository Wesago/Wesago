...
@patch("wesago.users.groups.remove_group_from_users_groups_ids")
def test_custom_group_is_deleted(
    mock_remove_group_from_users_groups_ids, admin_client, custom_group
):
    response = admin_client.post(
        reverse("wesago:admin:groups:delete", kwargs={"pk": custom_group.id})
    )
...
def test_protected_group_cant_be_deleted(admin_client, members_group, user):
    response = admin_client.post(
        reverse("wesago:admin:groups:delete", kwargs={"pk": members_group.id})
    )
...
def test_non_existing_group_cant_be_deleted(admin_client):
    response = admin_client.post(
        reverse("wesago:admin:groups:delete", kwargs={"pk": 404})
    )
