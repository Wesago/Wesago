...
class Notification(models.Model):
    ...
    category = models.ForeignKey(
        "wesago_categories.Category", blank=True, null=True, on_delete=models.CASCADE
    )
    thread = models.ForeignKey(
        "wesago_threads.Thread", blank=True, null=True, on_delete=models.CASCADE
    )
    thread_title = models.CharField(max_length=255, blank=True, null=True)
    post = models.ForeignKey(
        "wesago_threads.Post", blank=True, null=True, on_delete=models.CASCADE
    )

    created_at = models.DateTimeField(auto_now_add=True, db_index=True)

    def get_absolute_url(self) -> str:
        return reverse("wesago:notification", kwargs={"notification_id": self.id})
...
class WatchedThread(models.Model):
    ...
    def get_disable_emails_url(self):
        return reverse(
            "wesago:notifications-disable-email",
            kwargs={
