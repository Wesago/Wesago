...
def test_notification_view_returns_redirect_to_user_notification(rf, user, user_client):
    ...
    response = user_client.get(
        reverse("wesago:notification", kwargs={"notification_id": notification.id})
    )
...
def test_notification_view_returns_404_error_for_nonexisting_notification(user_client):
    response = user_client.get(
        reverse("wesago:notification", kwargs={"notification_id": 1})
    )
...
def test_notification_view_shows_permission_denied_page_to_guests(db, client):
    response = client.get(reverse("wesago:notification", kwargs={"notification_id": 1}))
