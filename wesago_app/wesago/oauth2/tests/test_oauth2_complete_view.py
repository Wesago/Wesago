...
def test_oauth2_complete_view_returns_404_if_oauth_is_disabled(
    ...
    response = client.get(reverse("wesago:oauth2-complete"))
...
def test_oauth2_complete_view_returns_error_400_if_user_canceled_sign_in(
    ...
    response = client.get("%s?error=access_denied" % reverse("wesago:oauth2-complete"))
...
def test_oauth2_complete_view_returns_error_400_if_state_is_invalid(
    ...
    response = client.get(
        "%s?state=invalid&code=1234" % reverse("wesago:oauth2-complete")
    )
    assert_contains(
        response,
...
def test_oauth2_complete_view_returns_error_400_if_code_is_missing(
    ...
    response = client.get("%s?state=state123&code=" % reverse("wesago:oauth2-complete"))
    assert_contains(
        response,
...
def test_oauth2_complete_view_creates_new_user(client, dynamic_settings, mailoutbox):
    ...
    response = client.get(
        "%s?state=%s&code=%s"
        % (
            reverse("wesago:oauth2-complete"),
            session_state,
            code_grant,
        )
    )
    ...
    auth_api = client.get(reverse("wesago:api:auth")).json()
    assert auth_api["id"] == user.id

    # User welcome e-mail is sent
    assert len(mailoutbox) == 1
    assert mailoutbox[0].subject == "Welcome on Wesago forums!"
