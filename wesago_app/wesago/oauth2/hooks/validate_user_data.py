...
class ValidateUserDataHookAction(Protocol):
    """
    A standard Wesago function used for validating the user data, or the next
...
class ValidateUserDataHookFilter(Protocol):
    ...
    A standard Wesago function used for filtering the user data, or the next
    filter function from another plugin.
...
class ValidateUserDataHook(
    ...
    """
    This hook wraps the standard function that Wesago uses to validate a Python
    ...
    from django.http import HttpRequest
    from wesago.oauth.hooks import validate_user_data_hook
    from wesago.users.models import User
