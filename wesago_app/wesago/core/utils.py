...
WESAGO_SLUGIFY = getattr(settings, "WESAGO_SLUGIFY", "wesago.core.slugify.default")

slugify = import_string(WESAGO_SLUGIFY)
...
def is_request_to_wesago(request):
    try:
        return request._request_to_wesago
    except AttributeError:
        request._request_to_wesago = _is_request_path_under_wesago(request)
        return request._request_to_wesago


def _is_request_path_under_wesago(request):
    # We are assuming that forum_index link is root of all Wesago links
    forum_index = reverse("wesago:index")
    path = request.path
