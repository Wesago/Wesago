...
from .utils import get_exception_message, is_request_to_wesago
...
def _error_page(request, code, exception=None, default_message=None):
    request.frontend_context.update({"CURRENT_LINK": "wesago:error-%s" % code})

    return render(
        request,
        "wesago/errorpages/%s.html" % code,
...
def banned(request, exception):
    ...
    request.frontend_context.update(
        {"MESSAGE": ban.get_serialized_message(), "CURRENT_LINK": "wesago:error-banned"}
    )

    return render(request, "wesago/errorpages/banned.html", {"ban": ban}, status=403)
...
def csrf_failure(request, reason=""):
    ...
    return render(request, "wesago/errorpages/csrf_failure.html", status=403)
...
def shared_403_exception_handler(f):
    def page_decorator(request, *args, **kwargs):
        if is_request_to_wesago(request):
