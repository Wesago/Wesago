...
from ..utils import (
    ...
    is_referer_local,
    is_request_to_wesago,
    parse_iso8601_string,
    slugify,
)


class IsRequestToWesagoTests(TestCase):
    def test_is_request_to_wesago(self):
        """
        is_request_to_wesago correctly detects requests directed at Wesago
        """
        VALID_PATHS = ("/", "/threads/")
        INVALID_PATHS = ("", "somewhere/")

        wesago_prefix = reverse("wesago:index")

        for path in VALID_PATHS:
            request = RequestFactory().get("/")
            request.path = path
            self.assertTrue(
                is_request_to_wesago(request),
                '"%s" is not overlapped by "%s"' % (path, wesago_prefix),
            )
        ...
        for path in INVALID_PATHS:
            ...
            self.assertFalse(
                is_request_to_wesago(request),
                '"%s" is overlapped by "%s"' % (path, wesago_prefix),
            )


class SlugifyTests(TestCase):
    def test_valid_slugify_output(self):
        """Wesago's slugify correctly slugifies string"""
        test_cases = [
            ("User", "user"),
...
PLAINTEXT_FORMAT_CASES = [
    ...
    (
        "http://wesago-project.org/login/",
        (
            "<p>"
            '<a href="http://wesago-project.org/login/">'
            "http://wesago-project.org/login/"
            "</a>"
            "</p>"
        ),
...
class CleanReturnPathTests(TestCase):
    def test_get_request(self):
        ...
        bad_request = MockRequest(
            "GET",
            {"HTTP_REFERER": "http://cookies.com", "HTTP_HOST": "wesago-project.org"},
        )
        ...
        bad_request = MockRequest(
            "GET",
            {
                "HTTP_REFERER": "https://wesago-project.org/",
                "HTTP_HOST": "wesago-project.org/",
            },
        )
        self.assertIsNone(clean_return_path(bad_request))
        ...
        bad_request = MockRequest(
            ...
            {
                "HTTP_REFERER": "https://wesago-project.org/",
                "HTTP_HOST": "wesago-project.org/assadsa/",
            },
        )
        self.assertIsNone(clean_return_path(bad_request))
        ...
        ok_request = MockRequest(
            "GET",
            {
                "HTTP_REFERER": "http://wesago-project.org/",
                "HTTP_HOST": "wesago-project.org/",
            },
        )
        ...
        ok_request = MockRequest(
            "GET",
            {
                "HTTP_REFERER": "http://wesago-project.org/login/",
                "HTTP_HOST": "wesago-project.org/",
            },
        )
