...
def is_wesago_exception(exception):
...
def get_exception_handler(exception):
    for exception_type, handler in EXCEPTION_HANDLERS:
        if isinstance(exception, exception_type):
            return handler
    raise ValueError("%s is not a Wesago exception" % exception.__class__.__name__)


def handle_wesago_exception(request, exception):
