...
class GetUserPermissionsHookAction(Protocol):
    """
    A standard Wesago function used to get user permissions.
...
class GetUserPermissionsHookFilter(Protocol):
    ...
    A standard Wesago function used to get user permissions or the next filter
...
class GetUserPermissionsHook(
    ...
    """
    This hook wraps the standard function that Wesago uses to get user permissions.
    ...
    ```python
    from django.contrib.auth.models import AnonymousUser
    from wesago.permissions.hooks import build_user_permissions_hook
