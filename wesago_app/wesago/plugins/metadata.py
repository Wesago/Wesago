...
class PluginsMetadataLoader:
    ...
    def build_plugin_metadata(self, plugin_package: str) -> PluginMetadata:
        ...
        manifest: Optional[WesagoPlugin] = None
        manifest_error: Optional[str] = None

        try:
            manifest_module = import_module(f"{plugin_package}.wesago_plugin")
            manifest_attr = getattr(manifest_module, "manifest", None)
            if manifest_attr is not None and not isinstance(
                manifest_attr, WesagoPlugin
            ):
                raise TypeError(
                    pgettext_lazy(
                        "plugins metadata",
                        "Plugin manifest is of an unexpected type '%(manifest_type)s' (expected WesagoPlugin instance)",
                    )
...
def create_plugin_metadata(
    ...
    manifest: Optional[WesagoPlugin],
    manifest_error: Optional[str],
) -> PluginMetadata:
    has_manifest = False
