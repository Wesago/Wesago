...
from ..manifest import WesagoPlugin
...
def test_plugin_metadata_is_created_without_manifest():
    ...
    assert metadata.dirname == "wesago"
...
def test_plugin_metadata_is_created_from_valid_full_manifest():
    manifest = WesagoPlugin(
        ...
        version="1.0",
        author="Lorem Ipsum",
        homepage="https://wesago-project.org",
        sponsor="https://sponsor.wesago-project.org",
        help="https://help.wesago-project.org",
        bugs="https://bugs.wesago-project.org",
        repo="https://repo.wesago-project.org",
    )
    ...
    assert metadata.homepage.netloc == "wesago-project.org"
    assert metadata.homepage.url == "https://wesago-project.org"
    assert metadata.sponsor.netloc == "sponsor.wesago-project.org"
    assert metadata.sponsor.url == "https://sponsor.wesago-project.org"
    assert metadata.help.netloc == "help.wesago-project.org"
    assert metadata.help.url == "https://help.wesago-project.org"
    assert metadata.bugs.netloc == "bugs.wesago-project.org"
    assert metadata.bugs.url == "https://bugs.wesago-project.org"
    assert metadata.repo.netloc == "repo.wesago-project.org"
    assert metadata.repo.url == "https://repo.wesago-project.org"
...
def test_plugin_url_returns_none_if_manifest_value_is_invalid_str():
    ...
    assert clean_plugin_url("wesago-project.org") is None
    assert clean_plugin_url("www.wesago-project.org") is None
