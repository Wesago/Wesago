...
@pytest.fixture
def sys_mock(mocker):
    mock = mocker.patch("wesago.plugins.discover.sys")
    mock.path = []
    return mock
...
def create_plugin(plugins_dir: str, dir_name: str, package_name: str) -> Path:
    ...
    plugin_wesago_marker_path = plugin_package_path / "wesago_plugin.py"
    plugin_wesago_marker_path.touch()

    return plugin_wesago_marker_path
...
def test_discover_plugins_skips_packages_without_wesago_plugin_py(sys_mock):
    with TemporaryDirectory() as plugins_dir:
        create_plugin(plugins_dir, "mock-plugin", "mock_plugin")
...
@pytest.mark.xfail(reason="requires wesago-pypi-plugin installed to pass")
def test_discover_plugins_reads_plugins_from_pip_install_if_it_exists(sys_mock):
    with TemporaryDirectory() as plugins_dir:
        create_pip_install(plugins_dir, ["wesago-pypi-plugin"])

        plugins = discover_plugins(plugins_dir)
        assert plugins == ["wesago_pypi_plugin"]
        assert sys_mock.path == []
...
def test_discover_plugins_skips_pip_install_entries_that_arent_installed(sys_mock):
    with TemporaryDirectory() as plugins_dir:
        create_pip_install(plugins_dir, ["wesago-pypi-plugin", "wesago-other-plugin"])
...
@pytest.mark.xfail(reason="requires wesago-pypi-plugin installed to pass")
def test_discover_plugins_skips_pip_install_entries_that_arent_wesago_plugins(sys_mock):
    with TemporaryDirectory() as plugins_dir:
        create_pip_install(plugins_dir, ["wesago-pypi-plugin", "django"])
