from django.apps import AppConfig
from social_core.backends.discord import DiscordOAuth2
from social_core.backends.facebook import FacebookOAuth2
from social_core.backends.github import GithubOAuth2
from social_core.backends.google import GoogleOAuth2
from social_core.backends.twitter import TwitterOAuth

from .providers import providers


class WesagoSocialAuthConfig(AppConfig):
    name = "wesago.socialauth"
    label = "wesago_socialauth"
    verbose_name = "Wesago Social Auth"

    def ready(self):
        # Register default providers
        from .admin.forms import (
            DiscordForm,
            FacebookForm,
            GitHubForm,
            GoogleForm,
            TwitterForm,
        )

        providers.add(
            provider="discord",
            name="Discord",
            auth_backend=DiscordOAuth2,
            settings={"scope": ["email"]},
            admin_form=DiscordForm,
            admin_template="wesago/admin/socialauth/form.html",
        )
        providers.add(
            provider="facebook",
            name="Facebook",
            auth_backend=FacebookOAuth2,
            settings={"scope": ["email"]},
            admin_form=FacebookForm,
            admin_template="wesago/admin/socialauth/form.html",
        )
        providers.add(
            provider="github",
            name="GitHub",
            auth_backend=GithubOAuth2,
            settings={"scope": ["read:user", "user:email"]},
            admin_form=GitHubForm,
            admin_template="wesago/admin/socialauth/form.html",
        )
        providers.add(
            provider="google-oauth2",
            name="Google",
            auth_backend=GoogleOAuth2,
            admin_form=GoogleForm,
            admin_template="wesago/admin/socialauth/form.html",
        )
        providers.add(
            provider="twitter",
            name="X",
            auth_backend=TwitterOAuth,
            admin_form=TwitterForm,
            admin_template="wesago/admin/socialauth/form.html",
        )
