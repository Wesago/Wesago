...
class ThreadMergeApiTests(ThreadsApiTestCase):
    def setUp(self):
        ...
        self.api_link = reverse(
            "wesago:api:thread-merge", kwargs={"pk": self.thread.pk}
        )
    ...
    @patch_category_acl({"can_merge_threads": True})
    @patch("wesago.threads.moderation.threads.delete_duplicate_watched_threads")
...
def delete_duplicate_watched_threads_mock(mocker):
    return mocker.patch(
        "wesago.threads.moderation.threads.delete_duplicate_watched_threads"
    )
...
def test_thread_merge_api_merges_notifications(
    ...
    response = user_client.post(
        reverse("wesago:api:thread-merge", kwargs={"pk": thread.pk}),
        json={
            "other_thread": other_thread.get_absolute_url(),
        },
    )
