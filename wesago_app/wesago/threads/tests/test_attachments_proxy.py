...
def assert_403(response):
    ...
    assert response["location"].endswith(settings.WESAGO_ATTACHMENT_403_IMAGE)


def assert_404(response):
    assert response.status_code == 302
    assert response["location"].endswith(settings.WESAGO_ATTACHMENT_404_IMAGE)
...
def test_proxy_redirects_to_404_image_for_nonexistant_attachment(db, client):
    response = client.get(
        reverse("wesago:attachment", kwargs={"pk": 1, "secret": "secret"})
    )
    assert_404(response)
...
def test_proxy_redirects_to_404_image_for_url_with_invalid_attachment_secret(
    ...
    response = client.get(
        reverse("wesago:attachment", kwargs={"pk": attachment.id, "secret": "invalid"})
    )
    assert_404(response)
