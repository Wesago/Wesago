...
class ThreadsMergeApiTests(ThreadsApiTestCase):
    def setUp(self):
        super().setUp()
        self.api_link = reverse("wesago:api:thread-merge")
    ...
    @patch_category_acl({"can_merge_threads": True})
    @patch("wesago.threads.api.threadendpoints.merge.delete_duplicate_watched_threads")
...
def delete_duplicate_watched_threads_mock(mocker):
    return mocker.patch(
        "wesago.threads.api.threadendpoints.merge.delete_duplicate_watched_threads"
    )
...
def test_threads_merge_api_merges_notifications(
    ...
    response = user_client.post(
        reverse("wesago:api:thread-merge"),
        json={
