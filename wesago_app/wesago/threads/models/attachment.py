...
class Attachment(PluginDataModel):
    ...
    def generate_new_secret(cls):
        return get_random_string(settings.WESAGO_ATTACHMENT_SECRET_LENGTH)
    ...
    def get_absolute_url(self):
        return reverse(
            "wesago:attachment", kwargs={"pk": self.pk, "secret": self.secret}
        )

    def get_thumbnail_url(self):
        if self.thumbnail:
            return reverse(
                "wesago:attachment-thumbnail",
    ...
    def set_image(self, upload):
        ...
        downscale_image = (
            thumbnail.size[0] > settings.WESAGO_ATTACHMENT_IMAGE_SIZE_LIMIT[0]
            or thumbnail.size[1] > settings.WESAGO_ATTACHMENT_IMAGE_SIZE_LIMIT[1]
        )
        strip_animation = fileformat == "gif"

        thumb_stream = BytesIO()
        if downscale_image:
            thumbnail.thumbnail(settings.WESAGO_ATTACHMENT_IMAGE_SIZE_LIMIT)
            if fileformat == "jpg":
