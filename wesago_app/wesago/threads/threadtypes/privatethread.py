...
class PrivateThread(ThreadType):
    ...
    def get_thread_absolute_url(self, thread, page=1):
        ...
        return reverse(
            "wesago:private-thread", kwargs={"slug": thread.slug, "pk": thread.pk}
        )

    def get_thread_last_post_url(self, thread):
        return reverse(
            "wesago:private-thread-last", kwargs={"slug": thread.slug, "pk": thread.pk}
        )

    def get_thread_new_post_url(self, thread):
        return reverse(
            "wesago:private-thread-new", kwargs={"slug": thread.slug, "pk": thread.pk}
        )

    def get_thread_api_url(self, thread):
        return reverse("wesago:api:private-thread-detail", kwargs={"pk": thread.pk})

    def get_thread_editor_api_url(self, thread):
        return reverse(
            "wesago:api:private-thread-post-editor", kwargs={"thread_pk": thread.pk}
        )

    def get_thread_posts_api_url(self, thread):
        return reverse(
            "wesago:api:private-thread-post-list", kwargs={"thread_pk": thread.pk}
        )

    def get_post_merge_api_url(self, thread):
        return reverse(
            "wesago:api:private-thread-post-merge", kwargs={"thread_pk": thread.pk}
        )

    def get_post_absolute_url(self, post):
        return reverse(
            "wesago:private-thread-post",
            kwargs={"slug": post.thread.slug, "pk": post.thread.pk, "post": post.pk},
        )

    def get_post_api_url(self, post):
        return reverse(
            "wesago:api:private-thread-post-detail",
            kwargs={"thread_pk": post.thread_id, "pk": post.pk},
        )

    def get_post_likes_api_url(self, post):
        return reverse(
            "wesago:api:private-thread-post-likes",
            kwargs={"thread_pk": post.thread_id, "pk": post.pk},
        )

    def get_post_editor_api_url(self, post):
        return reverse(
            "wesago:api:private-thread-post-editor",
            kwargs={"thread_pk": post.thread_id, "pk": post.pk},
        )

    def get_post_edits_api_url(self, post):
        return reverse(
            "wesago:api:private-thread-post-edits",
            kwargs={"thread_pk": post.thread_id, "pk": post.pk},
        )

    def get_post_read_api_url(self, post):
        return reverse(
            "wesago:api:private-thread-post-read",
            kwargs={"thread_pk": post.thread_id, "pk": post.pk},
        )

    def get_thread_watch_api_url(self, thread):
        return reverse(
            "wesago:apiv2:private-thread-watch", kwargs={"thread_id": thread.id}
        )
