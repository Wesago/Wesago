...
class Thread(ThreadType):
    ...
    def get_category_absolute_url(self, category):
        if category.level:
            return reverse(
                "wesago:category", kwargs={"pk": category.pk, "slug": category.slug}
            )

        return reverse("wesago:threads")
    ...
    def get_thread_absolute_url(self, thread, page=1):
        ...
        return reverse("wesago:thread", kwargs={"slug": thread.slug, "pk": thread.pk})

    def get_thread_last_post_url(self, thread):
        return reverse(
            "wesago:thread-last", kwargs={"slug": thread.slug, "pk": thread.pk}
        )

    def get_thread_new_post_url(self, thread):
        return reverse(
            "wesago:thread-new", kwargs={"slug": thread.slug, "pk": thread.pk}
        )

    def get_thread_best_answer_url(self, thread):
        return reverse(
            "wesago:thread-best-answer", kwargs={"slug": thread.slug, "pk": thread.pk}
        )

    def get_thread_unapproved_post_url(self, thread):
        return reverse(
            "wesago:thread-unapproved", kwargs={"slug": thread.slug, "pk": thread.pk}
        )

    def get_thread_api_url(self, thread):
        return reverse("wesago:api:thread-detail", kwargs={"pk": thread.pk})

    def get_thread_editor_api_url(self, thread):
        return reverse("wesago:api:thread-post-editor", kwargs={"thread_pk": thread.pk})

    def get_thread_merge_api_url(self, thread):
        return reverse("wesago:api:thread-merge", kwargs={"pk": thread.pk})

    def get_thread_poll_api_url(self, thread):
        return reverse("wesago:api:thread-poll-list", kwargs={"thread_pk": thread.pk})

    def get_thread_watch_api_url(self, thread):
        return reverse("wesago:apiv2:thread-watch", kwargs={"thread_id": thread.id})

    def get_thread_posts_api_url(self, thread):
        return reverse("wesago:api:thread-post-list", kwargs={"thread_pk": thread.pk})

    def get_poll_api_url(self, poll):
        return reverse(
            "wesago:api:thread-poll-detail",
            kwargs={"thread_pk": poll.thread_id, "pk": poll.pk},
        )

    def get_poll_votes_api_url(self, poll):
        return reverse(
            "wesago:api:thread-poll-votes",
            kwargs={"thread_pk": poll.thread_id, "pk": poll.pk},
        )

    def get_post_merge_api_url(self, thread):
        return reverse("wesago:api:thread-post-merge", kwargs={"thread_pk": thread.pk})

    def get_post_move_api_url(self, thread):
        return reverse("wesago:api:thread-post-move", kwargs={"thread_pk": thread.pk})

    def get_post_split_api_url(self, thread):
        return reverse("wesago:api:thread-post-split", kwargs={"thread_pk": thread.pk})
