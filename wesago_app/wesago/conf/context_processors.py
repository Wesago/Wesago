...
# Simple but hard to guess version signature for current Wesago version
# Used to cache-bust django-i18n.js URL in the browser
I18N_VERSION_SIGNATURE = sha256(
    (
        f"{wesago.__version__}{wesago.__released__}"
...
def conf(request):
    return {
        "BLANK_AVATAR_URL": (
            request.settings.blank_avatar or static(settings.WESAGO_BLANK_AVATAR)
        ),
        ...
        "THREADS_ON_INDEX": settings.WESAGO_THREADS_ON_INDEX,
...
def preload_settings_json(request):
    ...
    if request.settings.enable_oauth2_client:
        login_url = reverse("wesago:oauth2-login")
    else:
    ...
    preloaded_settings.update(
        {
            ...
            "LOGIN_API_URL": settings.WESAGO_LOGIN_API_URL,
    ...
    request.frontend_context.update(
        {
            ...
            "WESAGO_PATH": reverse("wesago:index"),
            "SETTINGS": preloaded_settings,
            "STATIC_URL": settings.STATIC_URL,
            "THREADS_ON_INDEX": settings.WESAGO_THREADS_ON_INDEX,
            "NOTIFICATIONS_API": reverse("wesago:apiv2:notifications"),
            "NOTIFICATIONS_URL": reverse("wesago:notifications"),
        }
    )

    if (
        request.user.is_authenticated
        and request.user.is_wesago_admin
        and request.settings.show_admin_panel_link_in_ui
    ):
        request.frontend_context["ADMIN_URL"] = reverse("wesago:admin:index")
