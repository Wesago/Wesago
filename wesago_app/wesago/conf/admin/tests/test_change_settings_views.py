...
def messages_mock(mocker):
    return mocker.patch("wesago.conf.admin.views.messages")
...
def test_analytics_settings_form_renders(admin_client):
    response = admin_client.get(reverse("wesago:admin:settings:analytics:index"))
    assert response.status_code == 200


def test_captcha_settings_form_renders(admin_client):
    response = admin_client.get(reverse("wesago:admin:settings:captcha:index"))
    assert response.status_code == 200


def test_general_settings_form_renders(admin_client):
    response = admin_client.get(reverse("wesago:admin:settings:general:index"))
    assert response.status_code == 200


def test_notifications_settings_form_renders(admin_client):
    response = admin_client.get(reverse("wesago:admin:settings:notifications:index"))
    assert response.status_code == 200


def test_oauth2_settings_form_renders(admin_client):
    response = admin_client.get(reverse("wesago:admin:settings:oauth2:index"))
    assert response.status_code == 200


def test_threads_settings_form_renders(admin_client):
    response = admin_client.get(reverse("wesago:admin:settings:threads:index"))
    assert response.status_code == 200


def test_users_settings_form_renders(admin_client):
    response = admin_client.get(reverse("wesago:admin:settings:users:index"))
    assert response.status_code == 200
