...
class CaptchaSettingsForm(SettingsForm):
    ...
    enable_stop_forum_spam = YesNoSwitch(
        ...
        help_text=pgettext_lazy(
            "admin captcha settings form",
            "Turning this option on will result in Wesago validating new user's e-mail and IP address against SFS database.",
        ),
    )
    stop_forum_spam_confidence = forms.IntegerField(
        ...
        help_text=pgettext_lazy(
            "admin captcha settings form",
            "SFS compares user e-mail and IP address with database of known spammers and assigns the confidence score in range of 0 to 100 that user is a spammer themselves. If this score is equal or higher than specified, Wesago will block user from registering and ban their IP address for 24 hours.",
        ),
        min_value=0,
        max_value=100,
    )
