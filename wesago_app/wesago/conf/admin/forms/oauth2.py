...
class OAuth2SettingsForm(SettingsForm):
    ...
    oauth2_enable_pkce = YesNoSwitch(
        ...
        help_text=pgettext_lazy(
            "admin oauth2 settings form",
            "Enabling this option will make Wesago's OAuth2 client use PKCE (Proof Key for Code Exchange), increasing the security of the authentication process. The OAuth 2 server must also support PKCE.",
        ),
        required=False,
    )
