...
class CreateGroupHookAction(Protocol):
    """
    A standard Wesago function used for creating a new user group or the next filter
...
class CreateGroupHook(FilterHook[CreateGroupHookAction, CreateGroupHookFilter]):
    """
    This hook wraps the standard function that Wesago uses to create a new user group.

    Wesago group creation logic is a thin wrapper for `Group.objects.create()` that
    ...
    ```python
    from django.http import HttpRequest
    from wesago.postgres.delete import delete_all
