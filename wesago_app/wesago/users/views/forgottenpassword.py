...
def request_reset(request):
    ...
    request.frontend_context.update(
        {"SEND_PASSWORD_RESET_API": reverse("wesago:api:send-password-form")}
    )
    return render(request, "wesago/forgottenpassword/request.html")
...
def reset_password_form(request, pk, token):
    ...
    try:
        ...
        return render(
            request,
            "wesago/forgottenpassword/error.html",
            {"message": e.args[0]},
            status=400,
        )

    api_url = reverse(
        "wesago:api:change-forgotten-password", kwargs={"pk": pk, "token": token}
    )

    request.frontend_context["CHANGE_PASSWORD_API"] = api_url
    return render(request, "wesago/forgottenpassword/form.html")
