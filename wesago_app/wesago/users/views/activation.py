...
def request_activation(request):
    request.frontend_context.update(
        {"SEND_ACTIVATION_API": reverse("wesago:api:send-activation")}
    )
    return render(request, "wesago/activation/request.html")
...
def activate_by_token(request, pk, token):
    ...
    try:
        ...
        return render(request, "wesago/activation/stopped.html", {"message": e.args[0]})
    except ActivationError as e:
        return render(
            request, "wesago/activation/error.html", {"message": e.args[0]}, status=400
        )
