...
class UserManager(BaseUserManager):
    ...
    def create_user(self, username, email=None, password=None, **extra_fields):
        ...
        extra_fields.setdefault("is_wesago_root", False)
        return self._create_user(username, email, password, **extra_fields)

    def create_superuser(self, username, email, password=None, **extra_fields):
        extra_fields.setdefault("is_staff", True)
        extra_fields.setdefault("is_superuser", True)
        extra_fields.setdefault("is_wesago_root", True)

        if extra_fields.get("is_staff") is not True:
            raise ValueError("Superuser must have is_staff=True.")
        if extra_fields.get("is_superuser") is not True:
            raise ValueError("Superuser must have is_superuser=True.")
        if extra_fields.get("is_wesago_root") is not True:
            raise ValueError("Superuser must have is_wesago_root=True.")
...
class User(AbstractBaseUser, PluginDataModel, PermissionsMixin):
    ...
    # Wesago stores user email in two fields:
    ...
    group = models.ForeignKey("wesago_users.Group", on_delete=models.deletion.PROTECT)
    groups_ids = ArrayField(models.PositiveIntegerField(), default=list)
    permissions_id = models.CharField(max_length=12)

    # Wesago's root admin status
    # Root admin can do everything in Wesago's admin panel but has no power in
    # Django's admin panel.
    is_wesago_root = models.BooleanField(default=False)

    title = models.CharField(max_length=255, null=True, blank=True)

    requires_activation = models.PositiveIntegerField(default=ACTIVATION_NONE)

    # Controls user access to the Django site (not used by Wesago)
    ...
    roles = models.ManyToManyField("wesago_acl.Role")
    ...
    class Meta:
        indexes = [
            *PluginDataModel.Meta.indexes,
            models.Index(
                name="wesago_user_is_staff_part",
                fields=["is_staff"],
                condition=Q(is_staff=True),
            ),
            models.Index(
                name="wesago_user_requires_acti_part",
                fields=["requires_activation"],
                condition=Q(requires_activation__gt=0),
            ),
            models.Index(
                name="wesago_user_is_deleting_a_part",
                fields=["is_deleting_account"],
                condition=Q(is_deleting_account=True),
            ),
            models.Index(
                name="wesago_user_is_wesago_root",
                fields=["is_wesago_root"],
                condition=Q(is_wesago_root=True),
            ),
            GinIndex(
                name="wesago_user_groups_ids",
                fields=["groups_ids"],
            ),
        ]
    ...
    @property
    def is_wesago_admin(self):
        return self.is_wesago_root or DefaultGroupId.ADMINS in self.groups_ids
    ...
    def get_absolute_url(self):
        return reverse("wesago:user", kwargs={"slug": self.slug, "pk": self.pk})
    ...
    def get_real_name(self):
        # Bug: https://github.com/rafalp/Wesago/issues/1352
        ...
        if not isinstance(self.profile_fields, dict):
            ...
            raise RuntimeError(
                f"'profile_fields' has wrong type! Please post this WHOLE message on https://github.com/rafalp/Wesago/issues/1352 "
    ...
    def get_unread_notifications_for_display(self) -> Optional[str]:
        ...
        if self.unread_notifications > settings.WESAGO_UNREAD_NOTIFICATIONS_LIMIT:
            return f"{settings.WESAGO_UNREAD_NOTIFICATIONS_LIMIT}+"
...
class AnonymousUser(DjangoAnonymousUser):
    is_wesago_admin = False
    is_wesago_root = False
    acl_key = "anonymous"
