...
class AvatarsStoreTests(TestCase):
    def test_store(self):
        ...
        self.assertEqual(len(avatars_dict), len(settings.WESAGO_AVATARS_SIZES))
        self.assertEqual(len(user.avatars), len(settings.WESAGO_AVATARS_SIZES))
        self.assertEqual(len(user.avatars), len(avatars_dict))

        for avatar in user.avatars:
            self.assertIn(avatar["size"], settings.WESAGO_AVATARS_SIZES)
        ...
        new_avatars_dict = {}
        for size in settings.WESAGO_AVATARS_SIZES:
