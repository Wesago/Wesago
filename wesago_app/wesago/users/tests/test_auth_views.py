...
class AuthViewsTests(TestCase):
    def test_auth_views_return_302(self):
        ...
        response = self.client.get(reverse("wesago:login"))
        self.assertEqual(response.status_code, 302)

        response = self.client.post(reverse("wesago:login"))
        self.assertEqual(response.status_code, 302)

        response = self.client.get(reverse("wesago:logout"))
        self.assertEqual(response.status_code, 302)

        response = self.client.post(reverse("wesago:logout"))
        self.assertEqual(response.status_code, 302)
    ...
    def test_login_view_redirect_to(self):
        ...
        response = self.client.post(
            reverse("wesago:login"), data={"redirect_to": "/redirect/"}
        )
        ...
        response = self.client.post(
            reverse("wesago:login"), data={"redirect_to": "wesago:users"}
        )
        ...
        response = self.client.post(
            reverse("wesago:login"), data={"redirect_to": "canada goose not url!"}
        )
        ...
        response = self.client.post(
            reverse("wesago:login"), data={"redirect_to": "łelcome!"}
        )
...
def test_login_view_returns_403_if_oauth_is_enabled(db, client):
    response = client.get(reverse("wesago:login"))
