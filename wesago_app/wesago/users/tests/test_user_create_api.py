...
class UserCreateTests(UserTestCase):
    ...
    def test_registration_creates_active_user(self):
        ...
        auth_json = self.client.get(reverse("wesago:api:auth")).json()
...
def test_new_registrations_validators_hook_is_used_by_registration_api(
    ...
    mocker.patch(
        "wesago.users.validators.hooks.new_registrations_validators", [validator]
    )

    response = client.post(
        reverse("wesago:api:user-list"),
        {
            "username": "User",
            "email": "user@example.com",
