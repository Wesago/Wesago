...
logger = logging.getLogger("wesago.users.datadownloads")
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        working_dir = settings.WESAGO_USER_DATA_DOWNLOADS_WORKING_DIR
        if not working_dir:
            self.stdout.write(
                "WESAGO_USER_DATA_DOWNLOADS_WORKING_DIR has to be set in order for "
