...
class UserMiddleware(MiddlewareMixin):
    def process_request(self, request):
        ...
        elif not request.user.is_wesago_admin:
...
class OnlineTrackerMiddleware(MiddlewareMixin):
    def process_request(self, request):
        if request.user.is_authenticated:
            try:
                request._wesago_online_tracker = request.user.online_tracker
            except Online.DoesNotExist:
                tracker.start_tracking(request, request.user)
        else:
            request._wesago_online_tracker = None

    def process_response(self, request, response):
        if hasattr(request, "_wesago_online_tracker"):
            online_tracker = request._wesago_online_tracker
