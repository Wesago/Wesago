...
class TestDjangoAdminUserForm(AdminTestCase):
    def setUp(self):
        ...
        self.edit_test_user_in_django_url = reverse(
            "admin:wesago_users_user_change", args=[self.test_user.pk]
        )
        self.edit_test_user_in_wesago_url = reverse(
            "wesago:admin:users:edit", args=[self.test_user.pk]
        )
    ...
    def test_wesago_admin_url_presence_in_user_edit_view(self):
        """the url to Wesago admin is present in Django admin user edit view"""
        response = self.client.get(self.edit_test_user_in_django_url)
        self.assertContains(response, self.edit_test_user_in_wesago_url)
        edit_from_wesago_short_desc = (
            UserAdminModel.get_edit_from_wesago_url.short_description
        )
        self.assertContains(response, edit_from_wesago_short_desc)

    def test_wesago_admin_url_presence_in_user_list_view(self):
        """the url to Wesago admin is present in Django admin user list view"""
        response = self.client.get(reverse("admin:wesago_users_user_changelist"))
        self.assertContains(response, self.edit_test_user_in_wesago_url)
