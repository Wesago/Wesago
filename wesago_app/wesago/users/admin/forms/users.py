...
class NewUserForm(BaseUserForm):
    ...
    def clean_group(self):
        ...
        if not self.request.user.is_wesago_root and data == DefaultGroupId.ADMINS:
    ...
    def clean_secondary_groups(self):
        ...
        if not self.request.user.is_wesago_root and DefaultGroupId.ADMINS in data:
...
class EditUserForm(BaseUserForm):
    ...
    is_wesago_root = YesNoSwitch(
        label=pgettext_lazy("admin user form", "Is root administrator"),
        help_text=pgettext_lazy(
            "admin user form",
            "Root administrators can sign in to the Wesago Admin Control Panel and change other users' admin status. You need to be a root administrator to change this field.",
        ),
        disabled=True,
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        if request_user.is_wesago_root and request_user != self.instance:
            self.fields["is_wesago_root"].disabled = False

        if self.instance.is_wesago_admin and not (
            request_user.is_wesago_root or request_user == self.instance
        ):
        ...
        if (
            ...
            or (self.instance.is_wesago_admin and not request_user.is_wesago_root)
        ):
    ...
    def clean_group(self):
        ...
        if self.request.user.is_wesago_root:
            return group
    ...
    def filter_queryset(self, criteria, queryset):
        ...
        if criteria.get("is_admin"):
            queryset = queryset.filter(
                Q(is_wesago_root=True) | Q(groups_ids__contains=[DefaultGroupId.ADMINS])
            )
