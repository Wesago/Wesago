...
class UserAdmin(generic.AdminBaseMixin):
    root_link = "wesago:admin:users:index"
    templates_dir = "wesago/admin/users"
    model = User
...
class UsersList(UserAdmin, generic.ListView):
    ...
    def action_activate(self, request, users):
        ...
        if not inactive_users:
            ...
            mail_users(
                inactive_users,
                mail_subject,
                "wesago/emails/activation/by_admin",
    ...
    def action_ban(
        ...
        return self.render(
            ...
            template_name="wesago/admin/users/ban.html",
        )
    ...
    def action_delete_accounts(self, request, users):
        for user in users:
            ...
            if user.is_wesago_admin:
                raise generic.MassActionError(
                    pgettext(
                        "admin users",
                        "%(user)s can't be deleted because they are a Wesago administrator.",
                    )
                    % {"user": user.username}
                )
            if user.is_staff:
...
class NewUser(UserAdmin, generic.ModelFormView):
    ...
    def handle_form(self, form, request, target):
        ...
        return redirect("wesago:admin:users:edit", pk=new_user.pk)
...
class EditUser(UserAdmin, generic.ModelFormView):
    ...
    def handle_form(self, form, request, target):
        ...
        if not form.fields["is_wesago_root"].disabled:
            target.is_wesago_root = form.cleaned_data["is_wesago_root"]
