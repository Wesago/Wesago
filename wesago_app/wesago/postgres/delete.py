...
def delete_all(model: Type[Model], **where):
    ...
    `DELETE FROM "wesago_users_user" WHERE "group_id" = 9;` query.
    `delete_all(User, pk=[1, 3, 4])` executes the
    `DELETE FROM "wesago_users_user" WHERE "id" IN (1, 3, 4);` query.

    Multiple kwargs are joined with the "AND" keyword:

    `delete_all(User, group_id=9, is_wesago_admin=False)` will execute the
    `DELETE FROM "wesago_users_user" WHERE "group_id" = 9 AND "is_wesago_admin" = FALSE;`
    query.
