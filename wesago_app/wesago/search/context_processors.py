...
def search_providers(request):
    ...
    try:
        ...
        # with non-wesago's anonymous user model that has no acl support
        return {}

    request.frontend_context["SEARCH_URL"] = reverse("wesago:search")
    request.frontend_context["SEARCH_API"] = reverse("wesago:api:search")
        ...
        request.frontend_context["SEARCH_PROVIDERS"].append(
            {
                ...
                "icon": provider.icon,
                "url": reverse(
                    "wesago:search", kwargs={"search_provider": provider.url}
                ),
                "api": reverse(
                    "wesago:api:search", kwargs={"search_provider": provider.url}
                ),
                "results": None,
                "time": None,
            }
